...
st.sidebar.subheader('Visualization using seaborn and plotly by pdm02')
...
# boxplots
st.header("Boxplot")
st.subheader("With a categorical variable - target [noDM, DM]")
## multi-column layput
...
with row2_1: 
    corr_x = st.selectbox("Correlation - X variable", options=df.columns, index=df.columns.get_loc("restecg"))
        
with row2_2: 
    corr_y = st.selectbox("Correlation - Y variable", options=df.columns, index=df.columns.get_loc("oldpeak"))
...
df2=df[["age","cp","thalach","exang","oldpeak","slope","ca","thal"]]
fig6 = plt.figure(figsize=(10,9))
sns.heatmap(df2[::-1].corr(), annot=True, vmin=-1, vmax=1, cmap='coolwarm')
st.pyplot(fig6)
